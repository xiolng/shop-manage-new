...
export default class ColorList {
  constructor (colors, step = 5) {
    this.colors = colors
    this.step = step
    const colorList = colors.map(v => this.gradientColor(v.start, v.end, this.step))
  ...
  gradientColor (startColor, endColor, step) {
    ...
    for (let i = 0; i < step; i++) {
      const hex = this.colorsHex('rgb(' + parseInt((sR * i + startR)) + ',' + parseInt((sG * i + startG)) + ',' + parseInt((sB * i + startB)) + ')')
      colorArr.push(hex)
    }
    return colorArr
  }
